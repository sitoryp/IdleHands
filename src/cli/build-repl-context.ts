...
import {
  getGitShortStat, parseChangedFileCount,
} from './init.js';
...
export function buildReplContext(deps: ReplContextDeps): ReplContext {
  ...
  const ctx: ReplContext = {
    ...
    async readServerHealth(force = false) {
      ...
      if (snap.unsupported) { ctx.healthUnsupported = true; return null; }
      if (!snap.ok) {
    ...
    async maybePrintTurnMetrics(stats) {
      const sample: PerfTurnSample = {
        ts: Date.now(), turn: stats.turn, ttftMs: stats.ttftMs, ttcMs: stats.ttcMs,
        promptTokens: stats.promptTokensTurn ?? 0, completionTokens: stats.completionTokensTurn ?? 0,
        ppTps: stats.ppTps, tgTps: stats.tgTps,
      };
      ...
      if (typeof tg === 'number' && Number.isFinite(tg) && tg < slowThreshold) {
        console.log(warnFmt(`[perf] Generation slowed to ${tg.toFixed(1)} t/s - context may be too large`, S));
      }
    },
    ...
    async maybeOfferAutoCommit(taskHint: string) {
      ...
      const ans = (await rl.question(`Commit these changes? [Y/n/edit message] (${short}) `)).trim().toLowerCase();
      if (ans === 'n' || ans === 'no') { ctx.lastAutoCommitOfferStat = short; return; }
      ...
      if (run.status !== 0) { console.log((String(run.stdout || '') + String(run.stderr || '')).trim() || 'git add failed'); return; }
      ...
      if (commit.status === 0) { console.log(out || `Committed: ${msg}`); ctx.lastAutoCommitOfferStat = ''; }
      else { console.log(out || 'git commit failed'); ctx.lastAutoCommitOfferStat = short; }
    },
    ...
    async saveCurrentSession() {
      const payload = {
        ...
        model: session.model, harness: session.harness,
        contextWindow: session.contextWindow, messages: session.messages,
        sessionName: sessionName || undefined,
      };
      const targets = new Set<string>([
        resumeFile, lastSessionPath(), projectSessionPath(projectDir(config)),
      ]);
    ...
    stopWatchMode(announce = true) {
      if (ctx.watchDebounceTimer) { clearTimeout(ctx.watchDebounceTimer); ctx.watchDebounceTimer = null; }
      for (const w of ctx.watchWatchers) { try { w.close(); } catch {} }
      ctx.watchWatchers = [];
    ...
    async startWatchMode(paths: string[], maxIterationsPerTrigger: number) {
      ...
      for (const absPath of resolved) {
        ...
        watcher.on('error', (e) => console.error(warnFmt(`[watch] ${absPath}: ${String((e as any)?.message ?? e)}`, S)));
    ...
    async runWatchPrompt(changeSummary: string) {
      ...
      if (ctx.watchRunInFlight) { ctx.watchRunQueued = true; return; }
      ctx.watchRunInFlight = true;
      const priorMaxIterations = config.max_iterations;
      config.max_iterations = Math.max(1, Math.min(config.max_iterations, ctx.watchMaxIterationsPerTrigger));
      try {
    ...
    async startIndexInBackground() {
      if (!session.vault || !session.lens) {
        console.log('Indexing requires Vault + Lens enabled. Remove --no-vault/--no-lens/--no-trifecta.');
        return;
      }
      if (ctx.indexRunning) {
        const elapsed = Math.max(0, Math.floor((Date.now() - ctx.indexStartedAt) / 1000));
        console.log(`Index already running (${elapsed}s): scanned=${ctx.indexProgress.scanned}, indexed=${ctx.indexProgress.indexed}, skipped=${ctx.indexProgress.skipped}`);
        return;
      }
      ...
      void runProjectIndex({
        projectDir: cwd, vault: session.vault, lens: session.lens,
        onProgress: (p) => { ctx.indexProgress = { scanned: p.scanned, indexed: p.indexed, skipped: p.skipped, current: p.current }; },
      }).then((result) => {
        ctx.indexLastResult = result;
        ctx.indexRunning = false;
        const elapsed = Math.max(0, Math.floor((Date.now() - ctx.indexStartedAt) / 1000));
        console.log(S.dim(`[index] done in ${elapsed}s — scanned ${result.filesScanned}, indexed ${result.filesIndexed}, skipped ${result.filesSkipped}, removed ${result.filesRemoved}`));
        if (result.warnings.length) for (const w of result.warnings) console.log(warnFmt(`[index] ${w}`, S));
    ...
    async printIndexStats() {
      if (!session.vault) { console.log('No index metadata found for this project. Run /index to build one.'); return; }
      const keys = projectIndexKeys(projectDir(config));
      const row = await session.vault.getLatestByKey(keys.metaKey, 'system');
      if (!row?.value) { console.log('No index metadata found for this project. Run /index to build one.'); return; }
      const meta = parseIndexMeta(row.value);
      if (!meta) { console.log('No index metadata found for this project. Run /index to build one.'); return; }
      ...
      const langs = Object.entries(meta.languages || {}).sort((a: any, b: any) => b[1] - a[1]).slice(0, 8).map(([k, v]) => `${k}:${v}`).join(', ');
      ...
      if (ctx.indexRunning) {
        const elapsed = Math.max(0, Math.floor((Date.now() - ctx.indexStartedAt) / 1000));
        console.log(`  Background job: running (${elapsed}s) scanned=${ctx.indexProgress.scanned} indexed=${ctx.indexProgress.indexed} skipped=${ctx.indexProgress.skipped}`);
        if (ctx.indexProgress.current) console.log(`  Current file: ${ctx.indexProgress.current}`);
      }
    },

    async clearIndex() {
      if (!session.vault) { console.log('Vault is disabled. /index requires Vault + Lens enabled.'); return; }
      const keys = projectIndexKeys(projectDir(config));
      ...
      console.log(`Index cleared: removed ${removedFiles} file entr${removedFiles === 1 ? 'y' : 'ies'} (+${removedMeta + removedSummary} metadata entries).`);
    },
  };
