...
export function ensureCommandsRegistered(): void {
  ...
  registered = true;
  registerAll([
    ...sessionCommands, ...runtimeCommands, ...modelCommands,
    ...editingCommands, ...projectCommands, ...trifectaCommands,
    ...toolCommands, ...antonCommands,
  ]);
}
...
export function buildReplContext(
  ...
  return {
    ...
    S: makeStyler(true), version: PKG_VERSION,
    shellMode: false, activeShellProc: null, statusBarEnabled: false,
    statusBar: {} as any, lastStatusLine: '', lastAutoCommitOfferStat: '',
    sessionStartedMs: Date.now(), changesBaselineMs: Date.now(),
    lastRunnableInput: '', perfSamples: [], healthUnsupported: false,
    lastHealthWarning: '', lastHealthSnapshot: null, lastHealthSnapshotAt: 0,
    pendingTemplate: null, customCommands: new Map(), vimState: null, enabled: true,
    watchPaths: [], watchMaxIterationsPerTrigger: 3, watchActive: false,
    watchWatchers: [], watchDebounceTimer: null, watchPendingChanges: new Set(),
    watchRunInFlight: false, watchRunQueued: false,
    indexRunning: false, indexStartedAt: 0,
    indexProgress: { scanned: 0, indexed: 0, skipped: 0 },
    indexLastResult: null, indexLastError: '',
    antonActive: false, antonAbortSignal: null, antonLastResult: null, antonProgress: null,
    shutdown: async () => { if (cleanupFn) await cleanupFn(); },
    readServerHealth: async () => null, maybePrintTurnMetrics: asyncNoop,
    maybeOfferAutoCommit: asyncNoop,
    saveCurrentSession: saveFn,
    startWatchMode: asyncNoop, stopWatchMode: noop, runWatchPrompt: asyncNoop,
    startIndexInBackground: asyncNoop, printIndexStats: asyncNoop, clearIndex: asyncNoop,
    confirm: async () => true,
  };
}
...
export async function runShellCommand(line: string, config: IdlehandsConfig): Promise<{ output: string; inject: boolean; rc: number; command: string }> {
  ...
  const child = spawn('bash', ['-lc', command], { cwd, env: { ...process.env, IDLEHANDS: '1' }, stdio: ['pipe', 'pipe', 'pipe'] });
  let out = '', err = '';
  const timeout = setTimeout(() => { try { child.kill('SIGKILL'); } catch {} }, 30_000);
  child.stdout.on('data', (c) => { out += String(c); });
  child.stderr.on('data', (c) => { err += String(c); });
  const rc = await new Promise<number>((res) => {
    child.on('close', (code) => { clearTimeout(timeout); res(code ?? 1); });
    child.on('error', () => { clearTimeout(timeout); res(1); });
  });
