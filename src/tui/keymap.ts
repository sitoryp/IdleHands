...
export function resolveAction(key: string): TuiAction | null {
  switch (key) {
    ...
    case "pagedown": return "scroll_down";
    case "up": return "history_prev";
    case "down": return "history_next";
    case "left": return "cursor_left";
    case "right": return "cursor_right";
    case "enter": return "send";
    case "C-j": return "insert_newline";
    case "M-enter": return "insert_newline";
    case "backspace": return "backspace";
    case "delete": return "delete_forward";
    case "home": return "cursor_home";
    case "end": return "cursor_end";
    default: return null;
  }
}
...
export function decodeRawInput(chunk: string): string[] {
  ...
  for (let i = 0; i < chunk.length; i++) {
    ...
    if (ch === "\u001b") {
      const c1 = chunk[i + 1];
      const c2 = chunk[i + 2];
      if (c1 === "[" && c2 === "A") { keys.push("up"); i += 2; continue; }
      if (c1 === "[" && c2 === "B") { keys.push("down"); i += 2; continue; }
      if (c1 === "[" && c2 === "C") { keys.push("right"); i += 2; continue; }
      if (c1 === "[" && c2 === "D") { keys.push("left"); i += 2; continue; }
      if (c1 === "O" && c2 === "A") { keys.push("up"); i += 2; continue; }
      if (c1 === "O" && c2 === "B") { keys.push("down"); i += 2; continue; }
      if (c1 === "O" && c2 === "C") { keys.push("right"); i += 2; continue; }
      if (c1 === "O" && c2 === "D") { keys.push("left"); i += 2; continue; }
      // CSI sequences with ~ terminator: \x1b[N~ (PageUp=5~, PageDown=6~, Home=1~, End=4~, Delete=3~)
      if (c1 === "[" && c2 !== undefined) {
        const c3 = chunk[i + 3];
        if (c3 === "~") {
          if (c2 === "5") { keys.push("pageup"); i += 3; continue; }
          if (c2 === "6") { keys.push("pagedown"); i += 3; continue; }
          if (c2 === "1") { keys.push("home"); i += 3; continue; }
          if (c2 === "4") { keys.push("end"); i += 3; continue; }
          if (c2 === "3") { keys.push("delete"); i += 3; continue; }
