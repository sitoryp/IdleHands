...
**Runtime:** Node **>=24**  
**Note:** repo currently has **no lint configured** (`npm run lint` prints a placeholder).
...
## 1.1 Critical security: credentials/secrets at rest (plaintext in config)
### Current risk
`runtimes.json` host connection schema accepts fields like `password` and `key_path`, and the runtime store currently emits a warning that `*_ref` secret references are **not supported yet**.  
This implies plaintext secrets may be placed in config or passed around in ways that can leak.

### Required fixes (implementable in PRs)
...
## 1.2 High security: avoid shell injection & quoting bugs in CLI SSH runner
### Current risk
...
## 1.4 Medium security: absolute paths & sensitive filesystem disclosure
### Current risk
...
## 1.5 Performance optimizations (safe, measurable)
...
**Acceptance criteria**
- There is exactly one “progress state machine” implementation used by all frontends.
- No UI front-end maintains duplicate tool-line dedupe logic.

---

## 2.2 Remove duplicated escalation detection/keyword logic
### Current state
...
## 2.3 Normalize “host command execution” as a reusable class
### Current state
...
**Fix:** PR-PERF-01 + PR-PERF-02.

---

# Single actionable execution plan (phased)

## Phase 0 — Guardrails & tooling (1–2 PRs)
...
## Phase 1 — Secrets & secure-at-rest (2–4 PRs)
...
## Phase 2 — Dedup by shared classes (3 PRs)
...
## Phase 3 — Performance improvements (2 PRs)
...
## Phase 4 — Test expansion + regression prevention (2 PRs)
...
**Done when:** key security and orchestration behaviors have regression tests.

---

# Deliverables checklist (what you’ll have at the end)
- ✅ Encrypted secrets store + ref-based config
- ✅ Unified progress UX across TUI/Telegram/Discord with one implementation
- ✅ No shell-built SSH in CLI/runtime paths
- ✅ Measurable improvements to tool performance on large repos
- ✅ CI guardrails: lint + typecheck + tests + basic security scanning
- ✅ Reduced copy/paste blocks (escalation detection, progress plumbing, host exec)

---

## Appendix — Quick “PR stack” overview
- ENG-01: ESLint + Prettier + CI gate
- SEC-05: audit/Dependabot/CodeQL
- SEC-01: secrets store + CLI
- SEC-02: redaction + tool integration
- SEC-03: CLI host exec refactor (no shell-built SSH)
- REL-01: scheduler in-flight lock
- DEDUP-01: Discord uses presenter+scheduler
- DEDUP-02: TUI uses presenter
- DEDUP-03: shared escalation module
- DEDUP-04: HostCommandRunner consolidation
- PERF-01: streamed fallback search
- PERF-02: streamed read_file for huge files
- TEST-01: unit + integration test suite expansion
